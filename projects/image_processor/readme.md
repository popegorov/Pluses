...
### Пример
`./image_processor input.bmp /tmp/output.bmp -crop 100 200 800 600 -gs -blur 0.5`

В этом примере
1. Загружается изображение из файла `input.bmp`
2. Обрезается до изображения с началом в x=100, y=200 с размером 800х600 пикселей
