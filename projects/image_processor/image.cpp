...
void WriteColor(std::ofstream &out, Image &im, size_t x, size_t y) {
    ...
    WriteByte(out, MakeUint8_t(im.GetPicture()[y][x].green));
    WriteByte(out, MakeUint8_t(im.GetPicture()[y][x].red));
}
...
size_t GetPadding(Image &im) {
    return CalculatePadding(im.GetWidth());
}
...
void Image::Load(std::ifstream &in) {
    ...
    GetPicture().assign(height, std::vector<Color>(width));
    for (size_t y = GetHeight(); y > 0; --y) {
        for (size_t x = 0; x < GetWidth(); ++x) {
            ReadColor(in, *this, x, y - 1);
        }
        SkipNBytes(in, GetPadding(*this));
    }
}

void Image::Save(std::ofstream &out) {
    SaveStandardInformation(out, GetWidth(), GetHeight(), GetPadding(*this));

    for (size_t y = GetHeight(); y >= 1; --y) {
        for (size_t x = 0; x < GetWidth(); ++x) {
            WriteColor(out, *this, x, y - 1);
        }
        WriteNBytes(out, GetPadding(*this));
    }
}
